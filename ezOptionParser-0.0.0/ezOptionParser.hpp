...
namespace ez {
    ...
    */
    ...
    bool ezOptionParser::exportFile(const char * filename, bool all) {
        ...
        for(size_t i=1; i < firstArgs.size(); ++i) {
    ...
    void ezOptionParser::getUsageDescriptions(std::string & usage, int width, Layout layout) {
        ...
        for(i=0; i < groups.size(); ++i) {
            ...
            for(j=0; j < groups[k]->flags.size()-1; ++j) {
                ...
                if (static_cast<int>(opts.size()) > width)
                    opts.append("\n");
            }
        ...
        for(size_t i=0; i < groups.size(); ++i) {
            ...
            if (g->isSet) {
                ...
                for(size_t j=0; j < g->args.size(); ++j) {
                    if ((g->expectArgs != -1) && (g->expectArgs != static_cast<int>(g->args[j]->size())))
                        badOptions.push_back(*g->flags[0]);
                }
            }
        }
    ...
    void ezOptionParser::prettyPrint(std::string & out) {
        ...
        out += "First Args:\n";
        for(int i=0; i < static_cast<int>(firstArgs.size()); ++i) {
        ...
        for(int i=0; i < n; ++i) {						
            ...
            out += "\n";
            // The flag names:
            for(int j=0; j < static_cast<int>(g->flags.size())-1; ++j) {
            ...
            if (g->isSet) {
                if (g->expectArgs) {
                    if (g->args.empty()) {
                        ...
                        for(int k=0; k < static_cast<int>(g->args.size()); ++k) {
        ...
        out += "\nLast Args:\n";
        for(int i=0; i < static_cast<int>(lastArgs.size()); ++i) {
            sprintf(tmp, "%d: %s\n", i+1, lastArgs[i]->c_str());
            out += tmp;
        }

        out += "\nUnknown Args:\n";
        for(int i=0; i < static_cast<int>(unknownArgs.size()); ++i) {
